...
def db_home_pg_handle():
    rand_actors = Actor.query.order_by(sql_func.random()).limit(config.constants.DB_HOME_ACTORS_SHOWN).all()
    rand_movies = Movie.query.order_by(sql_func.random()).limit(config.constants.DB_HOME_MOVIES_SHOWN).all()
...
def db_actors_pg_handle():
    ...
    actor_query = Actor.query.filter(Actor.name.like(query + "%"))
    ...
    offset = (page - 1) * shown
    if query == "":
        actors = actor_query.order_by(Actor.id).offset(offset).limit(shown).all()  # if no query is specified, sorts by ID. if this isn't happening then the actors shown will be ones with weird names with quotes at the front, not ideal.
    else:
        actors = actor_query.order_by(Actor.name).offset(offset).limit(shown).all()
...
def db_movies_pg_handle():
    query = request.args.get("query", "", str)
    page = request.args.get("page", 1, int)
    movie_query = Movie.query.filter(Movie.title.like(query + "%"))
    ...
    offset = (page - 1) * shown
    if query == "":
        movies = movie_query.order_by(Movie.id).offset(offset).limit(shown).all()
    else:
        movies = movie_query.order_by(Movie.title).offset(offset).limit(shown).all()
...
def db_edit_actor_form_handle():
    ...
    actor = Actor.query.get(int(a_id))
    if actor == None:
...
def db_edit_movie_form_handle():
    ...
    movie = Movie.query.get(int(m_id))
    if movie == None:
...
def db_bacon_page():
    ...
    if a1_id_str == a2_id_str == "":    # both empties, probably a page reload.
        ...
        if case == 1:
            ...
            a1 = Actor.query.get(a_id_1)
            a2 = Actor.query.get(a_id_2)
            if a1 == None or a2 == None:
                case = 1
            elif a1 == a2:
                case = 2    # case 2 : same actors.
                path_list = [Actor.query.get(a_id_1)]   # the actor is sent for jinja to display.
            else:
                ...
                if path_list:
                    ...
                    for path_node in path_list:
                        if path_node > 0:   # actor
                            node = Actor.query.get(path_node)
                            path_dict[path_node] = node
                        else:   # movie
                            node = Movie.query.get(-path_node)
                            path_dict[path_node] = node
                else:
...
def db_add_role_form_handle():
    ...
    if fail:
        return "Invalid ID(s)."
    if Role.query.filter((Role.actor_id == a_id) & (Role.movie_id == m_id)).first():
...
def ajax_delete_role():
    ...
    if a_id == 0 or m_id == 0:
        ...
        role = Role.query.filter((Role.actor_id == a_id) & (Role.movie_id == m_id)).first()
        if role == None:
            fail = True
        else:
